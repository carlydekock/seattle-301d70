...
function displayPics(){
  ...
  // console.log(rando);
  // DONE: In a sentence or two, explain why the previous line of code threw an error when we changed the letiable declaration from `const to `const`.
  // This threw an error because we are trying to access rando outside of the code block it was defined, and therefore it does not exist. When it was var, we are still inside the function so the variable is still defined and still exists, changing it to const means that once we are outside that code block (the while loop it is declared within) it will no longer be defined.
  console.log(viewed);
